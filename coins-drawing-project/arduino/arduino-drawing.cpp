...
const int SERVO_PIN3 = 10;

const int TARGET_ANGLE1 = 0;
const int TARGET_ANGLE2 = 90;
const int TARGET_ANGLE3 = 90;

const int LIFT_SERVO_ANGLE = -35;
const int LOWER_SERVO_ANGLE = 30;

const int MOVE_ANGLE1 = 45; //middle
const int MOVE_ANGLE3 = 45; //end-effector
...
// базовый метод для движений робота
...
// вычисление углов, парсинг команд и перемещение энд-эффектора
void moveToCoordinates(ParsedCoordinates coords) {
  ...
  if (isnan(q1) || isnan(q2) || isinf(q1) || isinf(q2)) {
        Serial.println("Ошибка вычислений q1/q2 - пропуск");
        return;
  }
  ...
  angle1 = constrain(angle1, MIN_ANGLE1, MAX_ANGLE1);
  angle3 = constrain(angle3, MIN_ANGLE3, MAX_ANGLE3);
...
// опустить сервопривод
void lowerServo() {
  servo2.setEaseTo(LOWER_SERVO_ANGLE);
  synchronizeAllServosStartAndWaitForAllServosToStop();
}

// перемещение на полученные углы
void moveToAngles(float angle1, float angle3) {
